...
const CreateJournal: React.FC = () => {
  ...
  const handleMoodChange = (e: { detail: { value: any } }) => {
    const moodVal = e.detail.value;
    setMoodValue(moodVal);
  };

  const handleTypeChange = (e: { detail: { value: any } }) => {
  ...
  const {
    mutate: addJournal,
    isLoading: loadingAddJournal,
    error: errorAddJournal,
  } = useMutation(postJournal, {
    onSuccess: () => {
      // Success actions
      refreshPage();
    },
